...
function App() {
  ...
  const createAndDownloadPdf = (selectedCountry) => {
    ...
    axios.post('https://react-country-server.herokuapp.com/create-pdf', {name, capital, flag, popu, region, subregion, curr, lang})
    .then(() => {
      return axios.get('https://react-country-server.herokuapp.com/fetch-pdf', {
        name: name,
        responseType: 'blob'
      });
    })
